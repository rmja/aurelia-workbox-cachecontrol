...
export class CacheControl {
  ...
  handle(
    urlOrResponse: string | { url: string; headers: Headers },
  ): CacheControlBuilder & Promise<void> {
    const url =
      typeof urlOrResponse === "string" ? urlOrResponse : urlOrResponse.url;
  ...
  async ensurePrincipal(principalId: string) {
    if (!principalId) {
      throw new Error(
        "No principal id is specified, use clearPrivate() if all private entries should be deleted",
      );
    }

    this.logger.debug(
      `Ensuring that private entries in cache belongs to '${principalId}'`,
    );
    ...
    await this.db.transaction("rw", this.db.affiliations, async () => {
      const entries = await this.db.affiliations
        .where(nameOf<AffiliationEntry>("principalId"))
        .notEqual(principalId)
        .toArray();
  ...
  async bustTags(...tags: string[]) {
    await this.ensureInitialized();
    const tagEntries = await this.db.tags
      .where(nameOf<TagEntry>("tag"))
      .anyOf(tags)
      .toArray();
    ...
    this.logger.debug(
      `Busted ${urls.length} urls from ${tags.length} tags`,
      urls,
      tags,
    );
  }
  ...
  async refresh(url: string) {
    ...
    await this.db.transaction("rw", this.db.expirations, async () => {
      ...
      if (expiration && expiration.slidingExpiration) {
        expiration.nextExpiration = DateTime.local()
          .plus(Duration.fromISO(expiration.slidingExpiration))
          .toJSDate();
  ...
  private async deleteExpiredTick() {
    ...
    const nextExpirationEntry = await this.db.expirations
      .orderBy(nameOf<ExpirationEntry>("nextExpiration"))
      .first();

    if (nextExpirationEntry) {
      this.trySetExpiration(
        DateTime.fromJSDate(nextExpirationEntry.nextExpiration),
      );
    } else {
  ...
  private async deleteExpired() {
    ...
    await this.db.transaction("rw", this.db.expirations, async () => {
      const expirations = await this.db.expirations
        .where(nameOf<ExpirationEntry>("nextExpiration"))
        .belowOrEqual(now)
        .toArray();
  ...
  public trySetExpiration(expiration: DateTime) {
    if (!this.nextExpiration.isValid || expiration < this.nextExpiration) {
      ...
      this.deleteExpiredTimerHandle = self.setTimeout(
        this.deleteExpiredTick,
        ttlMs,
      );

      this.logger.info(
        `Next expiration timer will be fired at ${this.nextExpiration.toString()}`,
        this.nextExpiration,
      );
    }
  }
  ...
  public async delete(urls: string[]) {
    ...
    await this.db.transaction("rw", this.db.tags, async () => {
      const entries = await this.db.tags
        .where(nameOf<TagEntry>("url"))
        .anyOf(urls)
        .toArray();
  ...
  private async buildPromise() {
    ...
    if (this._isPrivate) {
      if (!this.cacheControl.currentPrincipalId) {
        this.cacheControl.logger.warn(
          "No principal is currently configured. Use ensurePrincipal() before making anything private.",
        );
      ...
      promises.push(
        this.cacheControl.db.affiliations.put({
          url: this.url,
          principalId: this.cacheControl.currentPrincipalId,
        }),
      );
    }
    ...
    if (this.absoluteExpiration) {
      ...
      nextExpiration = DateTime.local().plus(
    ...
    if (nextExpiration.isValid) {
      promises.push(
        this.cacheControl.db.expirations.put({
          url: this.url,
          created: new Date(),
          nextExpiration: nextExpiration.toJSDate(),
          ...(this.absoluteExpiration && {
            absoluteExpiration: this.absoluteExpiration.toJSDate(),
          }),
          ...(this.slidingExpiration && {
            slidingExpiration: this.slidingExpiration.toISO()!,
          }),
        }),
      );
    }
